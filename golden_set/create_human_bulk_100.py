...
EXISTING_FILE = "golden_set/human_validation_50_reviewed.csv"
OUTPUT_FILE = "golden_set/human_bulk_100.csv"
...
print("=" * 70)
print("CREATING 100 NEW HUMAN-LABEL REVIEW EXAMPLES")
print("=" * 70)
...
# --------------------------------------------------
# Remove examples already used in human validation
# --------------------------------------------------
...
if os.path.exists(EXISTING_FILE):
    ...
    print(f"Excluded existing human-validation tweets: {len(used_ids)}")
...
# --------------------------------------------------
# Stratified sampling using candidate groups
# --------------------------------------------------
...
df["candidate_group"] = df["text"].apply(candidate_group)

# --------------------------------------------------
# Sample approximately evenly across groups
# --------------------------------------------------
...
available_groups = [
    g for g in INTENTS
    if (df["candidate_group"] == g).sum() > 0
]

per_group = SAMPLE_SIZE // len(available_groups)
remainder = SAMPLE_SIZE % len(available_groups)
...
# If we somehow sampled fewer than 100, fill randomly
if len(result) < SAMPLE_SIZE:

    remaining = df[
        ~df["tweet_id"].isin(result["tweet_id"])
    ]
...
print(f"Examples created: {len(result)}")
